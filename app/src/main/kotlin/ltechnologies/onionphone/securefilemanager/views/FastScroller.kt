package ltechnologies.onionphone.securefilemanager.views
...
class FastScroller : FrameLayout {
        ...
        recyclerView.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrolled(rv: RecyclerView, dx: Int, dy: Int) {
                if (isScrollingEnabled) {
                    ...
                    currScrollX =
                        getValueInRange(0, recyclerViewContentWidth, currScrollX.toFloat()).toInt()
                    currScrollY =
                        getValueInRange(0, recyclerViewContentHeight, currScrollY.toFloat()).toInt()
        ...
        if (isHorizontal) {
            ...
            if (bubble != null && handle!!.isSelected) {
                ...
                bubble!!.x = getValueInRange(
                    tinyMargin,
                    recyclerViewWidth - bubbleWidth,
                    handle!!.x - bubbleWidth
                )
                bubbleHideHandler.removeCallbacksAndMessages(null)
                bubble?.alpha = 1f
            }
            ...
            if (bubble != null && handle!!.isSelected) {
                bubble!!.y = getValueInRange(
                    tinyMargin,
                    recyclerViewHeight - bubbleHeight,
                    handle!!.y - bubbleHeight
                )
                bubbleHideHandler.removeCallbacksAndMessages(null)
                bubble?.alpha = 1f
            }
    ...
    private fun getValueInRange(min: Int, max: Int, value: Float) =
        min(max(min.toFloat(), value), max.toFloat())
