...
import ltechnologies.onionphone.securefilemanager.interfaces.RecyclerScrollCallback
        ...
        when (ev.action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                if (dragSelectActive) {
                    ...
                    if (hotspotHeight > -1) {
                        if (ev.y in (hotspotTopBoundStart..hotspotTopBoundEnd).map { it.toFloat() }) {
                            inBottomHotspot = false
                            if (!inTopHotspot) {
                                inTopHotspot = true
                                autoScrollHandler.removeCallbacks(autoScrollRunnable)
                        ...
                        } else if (ev.y in (hotspotBottomBoundStart..hotspotBottomBoundEnd).map { it.toFloat() }) {
                            inTopHotspot = false
                            if (!inBottomHotspot) {
