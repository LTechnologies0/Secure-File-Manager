...
/** Decodes a file-backed bitmap with [maxDimension] cap to avoid OOM on large photos. */
fun String.decodeSampledBitmap(maxDimension: Int = 1024): Bitmap? {
    ...
    val sampleSize = maxOf(
        ...
        Integer.highestOneBit(
            maxOf(bounds.outWidth / maxDimension, bounds.outHeight / maxDimension),
        ),
    )
...
fun String.isEncrypted() = isOpenPgpFile()

fun String.isOpenPgpFile(): Boolean {
    val lower = lowercase(Locale.getDefault())
    return lower.endsWith(".gpg") || lower.endsWith(".pgp") || lower.endsWith(".asc")
}
