...
import ltechnologies.onionphone.securefilemanager.activities.AuthenticationActivity
...
val Context.isBiometricPresent: Boolean
    get() = BiometricManager.from(this).canAuthenticate(BIOMETRIC_AUTH) == BiometricManager.BIOMETRIC_SUCCESS
...
fun Context.hasPermission(permId: Int) = when {
    permId == PERMISSION_WRITE_STORAGE && Build.VERSION.SDK_INT >= Build.VERSION_CODES.R ->
...
fun Activity.requestStorageAccess(requestCode: Int = GENERIC_PERM_HANDLER) {
...
fun Context.getFilePublicUri(file: File, applicationId: String): Uri {
    if (file.absolutePath.isOpenPgpFile()) {
...
private fun Context.isProviderLocalPath(file: File): Boolean {
    val path = file.canonicalPath
    return path.startsWith(filesDir.canonicalPath) || path.startsWith(cacheDir.canonicalPath)
}
...
fun Context.getVideoResolution(path: String): Point? {
    var point = try {
        ...
        val width =
            retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_VIDEO_WIDTH)?.toInt() ?: return null
        val height =
            retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_VIDEO_HEIGHT)?.toInt() ?: return null
        Point(width, height)
