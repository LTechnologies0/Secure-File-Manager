...
open class Config(val context: Context) {
    ...
    var requireAuthForFileCrypto: Boolean
        get() = prefs.getBoolean(SETTINGS_REQUIRE_AUTH_FOR_CRYPTO, false)
        set(requireAuthForFileCrypto) = prefs.edit()
            .putBoolean(SETTINGS_REQUIRE_AUTH_FOR_CRYPTO, requireAuthForFileCrypto)
            .apply()
    ...
    var useGridView: Boolean
        ...
        set(useGridView) = prefs.edit().putBoolean(SETTINGS_USE_GRID_VIEW, useGridView).apply()

    var documentsProviderEnabled: Boolean
        get() = prefs.getBoolean(SETTINGS_DOCUMENTS_PROVIDER, false)
        set(documentsProviderEnabled) = prefs.edit()
            .putBoolean(SETTINGS_DOCUMENTS_PROVIDER, documentsProviderEnabled).apply()

    var showHiddenFiles: Boolean
        get() = prefs.getBoolean(SETTINGS_SHOW_HIDDEN_FILES, false)
        set(showHiddenFiles) = prefs.edit()
            .putBoolean(SETTINGS_SHOW_HIDDEN_FILES, showHiddenFiles).apply()

    var openFilesInternally: Boolean
        get() = prefs.getBoolean(SETTINGS_OPEN_INTERNAL, true)
        set(openFilesInternally) = prefs.edit()
            .putBoolean(SETTINGS_OPEN_INTERNAL, openFilesInternally).apply()

    var dualPaneEnabled: Boolean
        get() = prefs.getBoolean(
            SETTINGS_DUAL_PANE,
            context.resources.getBoolean(R.bool.dual_pane_default),
        )
        set(dualPaneEnabled) = prefs.edit()
            .putBoolean(SETTINGS_DUAL_PANE, dualPaneEnabled).apply()
