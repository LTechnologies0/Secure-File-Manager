...
import ltechnologies.onionphone.securefilemanager.dialogs.DeleteAppDataDialog
import ltechnologies.onionphone.securefilemanager.dialogs.PasswordSetupDialog
import ltechnologies.onionphone.securefilemanager.dialogs.RemoteServersDialog
import ltechnologies.onionphone.securefilemanager.dialogs.TrashRestoreDialog
...
import ltechnologies.onionphone.securefilemanager.providers.SfmDocumentsProvider
...
abstract class SettingsAbstractFragment : PreferenceFragmentCompat() {
    ...
    protected fun initFontSize(): ListPreference =
        ListPreference(requireContext()).apply {
            val activity = requireActivity()
            val config = activity.config
            val fontSizes = fontSizes(requireContext())
            val fontSizesStrings = fontSizes.keys.toTypedArray()
            val fontSize = requireContext().getFontSizeText()
    ...
    protected fun initEncryptionSettings(): Preference =
        Preference(requireContext()).apply {
            key = SETTINGS_ENCRYPTION
            title = getString(R.string.encryption_settings_title)
            summary = getString(R.string.encryption_settings_summary)
            setOnPreferenceClickListener {
                (requireActivity() as ltechnologies.onionphone.securefilemanager.activities.BaseAbstractActivity)
                    .supportFragmentManager
                    .beginTransaction()
                    .replace(
                        R.id.frame_layout,
                        SettingsEncryptionFragment(),
                    )
                    .addToBackStack(null)
                    .commit()
                true
    ...
    protected fun initOpenTrash(): Preference =
        Preference(requireContext()).apply {
            key = SETTINGS_OPEN_TRASH
            title = getString(R.string.trash_title)
            summary = getString(R.string.trash_summary)
            setOnPreferenceClickListener {
    ...
    protected fun initRemoteStorage(): Preference =
        Preference(requireContext()).apply {
    ...
    protected fun initTrashRestore(): Preference =
        Preference(requireContext()).apply {
            key = SETTINGS_TRASH_RESTORE
            title = getString(R.string.trash_restore_title)
            summary = getString(R.string.trash_restore_summary)
            setOnPreferenceClickListener {
                TrashRestoreDialog(requireActivity() as ltechnologies.onionphone.securefilemanager.activities.BaseAbstractActivity)
                true
            }
        }

    protected fun initDocumentsProvider(): SwitchPreferenceCompat =
        SwitchPreferenceCompat(requireContext()).apply {
            val activity = requireActivity()
            val config = activity.config
            key = SETTINGS_DOCUMENTS_PROVIDER
            title = getString(R.string.documents_provider_title)
            summaryOn = getString(R.string.documents_provider_summary_on)
            summaryOff = getString(R.string.documents_provider_summary_off)
            isChecked = config.documentsProviderEnabled
            setOnPreferenceChangeListener { _, newValue ->
                val enabled = newValue as Boolean
                config.documentsProviderEnabled = enabled
                SfmDocumentsProvider.setEnabled(activity, enabled)
                true
            }
        }
    ...
    protected fun initBlockScreenshots(): SwitchPreferenceCompat =
        SwitchPreferenceCompat(requireContext()).apply {
            ...
            setOnPreferenceChangeListener { _, newValue ->
                ...
                icon = getBlockScreenshotsIcon()
                activity.applyScreenshotBlockPolicy()
                true
            }
        }
    ...
    private fun initAppLock(): SwitchPreferenceCompat =
        SwitchPreferenceCompat(requireContext()).apply {
            ...
            setOnPreferenceChangeListener { _, newValue ->
                ...
                if (isChecked) {
                    activity.config.wasAppProtectionHandled = false
                }
    ...
    private fun initOpenBiometricSettings(): Preference =
        Preference(requireContext()).apply {
            ...
            setOnPreferenceClickListener {
                requireActivity().openAndroidSystemSettings()
                true
            }
        }
